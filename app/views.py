...
from .models import db, Staff, Client, Year, PaymentData, Month
...
def dashboard():
    ...
    for payment in all_payment:
        for count in range(len(all_payment)):
            if count < 5 and payment.last_edit_date == date.today() - timedelta(
                days=count
            ):
    ...
    return render_template(
        "dashboard.html",
        client_data=all_client,
        staff_data=all_staff,
        history_data=client_data_last_5_day,
        month_data=all_month,
...
def edit_profile():
    client_name = (
        f"{current_user.first_name} {current_user.last_name} ({current_user.username})"
    )
    ...
    if request.method == "GET":
        return render_template(
            "editprofile.html", client_name=client_name, current_staff=current_user
        )
...
def client_list():
    if request.method == "GET":
        ...
        return render_template(
            "clientlist.html", clients=client_list, current_staff=current_user
        )

    search_value = request.form.get("search_value")
    if search_value:
        return redirect(
            url_for("views.search_client", from_loc="client_list", value=search_value)
        )
...
def edit_client(client_id):
    if request.method == "GET":
        ...
        return render_template(
            "/views/editclient.html", current_staff=current_user, client=client
        )
...
def archive_list():
    if request.method == "GET":
        client_list = Client.query.order_by(Client.first_name).all()
        return render_template(
            "archivelist.html", clients=client_list, current_staff=current_user
        )

    search_value = request.form.get("search_value")
    if search_value:
        return redirect(
            url_for("views.search_client", from_loc="archive_list", value=search_value)
        )
...
def search_client(from_loc, value):
    ...
    for client in raw_client_list:
        ...
        if value.lower() in name_str:
            client_list.append(client)

    if from_loc == "client_list":
        return render_template(
            "/operation_template/search_client_list.html",
            clients=client_list,
            current_staff=current_user,
            search_value=value,
            is_search=True,
    ...
    if from_loc == "archive_list":
        return render_template(
            "/operation_template/search_archive_list.html",
            clients=client_list,
            current_staff=current_user,
            search_value=value,
            is_search=True,
...
def add_client_data():
    ...
    new_client = Client(
        first_name=form_firstname, last_name=form_lastname, call_name=form_callname
    )
    ...
    try:
        ...
        flash(
            f"Client dengan nama panggilan {form_callname} sudah ada!", category="error"
        )
...
def data_pembayaran(client_id):
    ...
    return render_template(
        "datapembayaran.html",
        client=client,
        year_button_display="Pilih Tahun",
        year_list=year_list,
        current_staff=current_user,
    )
...
def display_data_pembayaran(client_id, year):
    ...
    for num in range(12):
        old_payment_data = PaymentData.query.filter_by(
            payer=client.id, paid_year=year_object.id, paid_month=month_in_year[num].id
        ).first()
        form_payment_state = request.form.get(f"month{month_in_year[num].id}")
        form_payment_paid_date = request.form.get(
            f"tanggal_bayar{month_in_year[num].id}"
        )
        tanggal_deadline_pembayaran = request.form.get(
            "tanggal-deadline-pembayaran-tiap-bulan"
        )

        # cek apakah Client sudah mempunyai deadline pembayaran atau belum,
        # jika belum tambahkan tanggal pembayaran, jika sudah abaikan, jika berbeda update
        if not client.deadline_pembayaran:
            client.deadline_pembayaran = tanggal_deadline_pembayaran
        else:
            if client.deadline_pembayaran != tanggal_deadline_pembayaran:
                client.deadline_pembayaran = tanggal_deadline_pembayaran
        db.session.commit()
        ...
        if form_payment_state == "lunas" and old_payment_data.states != "lunas":
            if not form_payment_paid_date:
                ...
                return redirect(
                    url_for(
                        "views.display_data_pembayaran", client_id=client_id, year=year
                    )
                )
        ...
        if form_payment_state != old_payment_data.states:
            update_payment_data = PaymentData.query.filter_by(
                payer=client.id,
                paid_year=year_object.id,
                paid_month=month_in_year[num].id,
            ).update(
                {
                    PaymentData.states: form_payment_state
                    if old_payment_data.states != form_payment_state
                    else old_payment_data.states,
                    PaymentData.date_paid: form_payment_paid_date
                    if form_payment_paid_date
                    else old_payment_data.date_paid,
                    PaymentData.last_edit_by: current_user.id
                    if old_payment_data.last_edit_by != current_user.id
                    else old_payment_data.last_edit_by,
                    PaymentData.last_edit_date: datetime.now(),
                    PaymentData.isPaid: True
                    if form_payment_state == "lunas"
                    else False,
                }
            )
        db.session.commit()
